...
pub fn render_bloom_extract_pass(encoder: &mut CommandEncoder, resources: &RenderResources) {
    let (target_view, bloom_buffer_bind_group) = resources.targets.begin_bloom_pass();
    let hdr_buffer_bind_group = resources.targets.hdr_buffer_bind_group();
    ...
    render_pass.set_pipeline(resources.post_process.pipelines.bloom_extract_pipeline());

    render_pass.set_bind_group(
        0,
        resources.post_process.bind_groups.bloom_bind_group(),
        &[],
    );
